...
```
 ...
 │ MariaDB     │  │ SQL schema  │  │ RADIUS only │
 └─────────────┘  └─────────────┘  └─────────────┘
        │
        ▼
 ┌─────────────┐
...
| Database   | MySQL / MariaDB (`isp_billing` main + `radius` schema) |
...
- **Administration** — Users, Roles & Permissions (granular RBAC), System Settings, Audit Logs, Backup & Restore (mysqldump).
...
The RADIUS connection uses the MySQL/MariaDB FreeRADIUS schema
