...
#   - Composer 2
#   - MySQL 8 server + client (started and enabled)
...
This script automates Debian/Ubuntu. On RHEL-family systems install:
  ...
  systemctl enable --now mysqld crond
...
EOF
    exit 1
    ;;
  *) die "Unsupported distribution: ${PRETTY_NAME:-unknown}. Install PHP 8.2+, Composer, MySQL 8 and Node 18.18+ manually, then run ./install.sh." ;;
esac
...
apt-get install -y -qq mysql-server mysql-client >/dev/null
if command -v systemctl >/dev/null 2>&1 && [ -d /run/systemd/system ]; then
  systemctl enable --now mysql >/dev/null 2>&1 || service mysql start
else
  service mysql start 2>/dev/null || true
fi
ok "MySQL $(mysql --version | awk '{print $3}') installed and running"
...
──────────────────────────────────────────────────────────────
 Core dependencies installed:
   ...
   MySQL      $(mysql --version | awk '{print $3}')   (service running)
